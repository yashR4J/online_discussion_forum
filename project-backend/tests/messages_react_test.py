...
#React to messages
    ...
    for r in messages3['reacts']:
        if r['react_id'] == REACT:
            assert setup['user2']['auth_user_id'] in r['u_ids']
            assert get_notifications_v1(setup['user2']['token'])[0]['notification_message'] == 'stevenjacobs reacted to your message in stevejobs,stevenjacobs,thomasblack'
    ...
    for r in messages1['reacts']:
        if r['react_id'] == REACT:
            assert setup['user3']['auth_user_id'] in r['u_ids']
            assert get_notifications_v1(setup['user2']['token'])[0]['notification_message'] == 'thomasblack reacted to your message in channel2'
...
#Unreact to messages
    ...
    for r in messages3['reacts']:
        if r['react_id'] == REACT:
            assert r['u_ids'] == []
            assert setup['user2']['auth_user_id'] not in r['u_ids']
    ...
    for r in messages1['reacts']:
        if r['react_id'] == REACT:
            assert r['u_ids'] == []
            assert setup['user3']['auth_user_id'] not in r['u_ids']
