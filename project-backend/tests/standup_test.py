...
def test_standup_send_v1(setup):
    ...
    # Before standup begins, send a message to an inactive standup (expecting InputError)
    ...
    with pytest.raises(InputError):
        assert standup_send_v1(setup['sample_user1']['token'], setup['sample_channel_id1'], message)
    ...
    # Check standup is running (so multiple standups cannot start)
    with pytest.raises(InputError):
        standup_start_v1(setup['sample_user1']['token'], setup['sample_channel_id1'], 0.001)

    # Send a long (invalid) message
    long_message = "Sed ut perspiciatis unde omnis iste natus error sit voluptatem" * 1000
    # Test message greater than 1000 characters and empty string
    with pytest.raises(InputError):
        assert standup_send_v1(setup['sample_user1']['token'], setup['sample_channel_id1'], long_message)
