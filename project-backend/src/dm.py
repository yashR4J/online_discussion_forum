...
def dm_details_v1(token: Union[str, bytes], dm_id: int) -> dict:
    '''
    ...
    user_info_list = [helper.user_info(member) for member in dms[dm_id]['members']]

    return {'name': dms[dm_id]['name'], 'members': user_info_list}


def dm_list_v1(token: Union[str, bytes]) -> list:
    '''
...
def dm_create_v1(token: Union[str, bytes], u_ids: list) -> dict:
    '''
...
def dm_remove_v1(token: Union[str, bytes], dm_id: int) -> dict:
    '''
...
def dm_invite_v1(token: Union[str, bytes], dm_id: int, u_id: int) -> dict:
    '''
    ...
    # Add user with u_id
    if u_id not in dms[dm_id]['members'] and u_id != dms[dm_id]['owner']:
...
def dm_leave_v1(token: Union[str, bytes], dm_id: int) -> dict:
    '''
...
def dm_messages_v1(token: Union[str, bytes], dm_id: int, start: int) -> dict:
    '''
    ...
    # Change 'is_this_user_reacted'
    u_id = helper.check_token(token)['auth_user_id']
    for m in dms[dm_id]['messages']:
        if u_id in message_list:
            m['reacts'][0]['is_this_user_reacted'] = True
        else:
            m['reacts'][0]['is_this_user_reacted'] = False
