...
from src.dm import dm_create_v1, dm_details_v1, dm_list_v1, dm_remove_v1, \
    dm_leave_v1, dm_messages_v1, dm_invite_v1
...
def auth_register():
    ...
    ret = auth_register_v1(incoming['email'], \
                    incoming['password'], incoming['name_first'], \
                    incoming['name_last'])

    # Upload Default Profile Photo
    user_profile_uploadphoto(ret['token'], \
    'https://t4.ftcdn.net/jpg/00/64/67/63/360_F_64676383_LdbmhiNM6Ypzb3FM4PPuFP9rHe7ri8Ju.jpg', \
    0, 0, 360, 360)

    return saveAndReturn(ret)
...
def auth_passwordreset_request():
    ...
    '''
    incoming = request.get_json()
    return saveAndReturn(auth_passwordreset_request_v1(incoming['email']))
...
def auth_passwordreset_reset():
    ...
    '''
    incoming = request.get_json()
    return saveAndReturn(auth_passwordreset_reset_v1(incoming ['reset_code'], incoming['new_password']))
...
def set_user_stats_v1():
    '''
    Flask wrapper for user_stats_v1 that takes token
    '''
    return saveAndReturn(user_stats_v1(request.args.get("token")))

@APP.route("/users/stats/v1", methods=['GET'])
def set_users_stats_v1():
    '''
    Flask wrapper for users_stats_v1 that takes incoming token
    '''
    return saveAndReturn(users_stats_v1(request.args.get("token")))

@APP.route("/user/profile/uploadphoto/v1", methods=['POST'])
def set_user_profile_uploadphoto():
    '''
    Flask wrapper for user_profile_uploadphoto that takes token, image url, x_start, y_start, x_end, y_end to upload user profile photo.
    '''
    incoming = request.get_json()
    return saveAndReturn(user_profile_uploadphoto(incoming['token'], \
                            incoming['img_url'], incoming['x_start'], \
                            incoming['y_start'], incoming['x_end'], incoming['y_end']))

@APP.route("/static/<path:path>", methods=['GET'])
def user_profile_image(path):
    """
