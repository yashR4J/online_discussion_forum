...
from src.other import notify, notify_react
...
def message_send_v1(token: Union[str, bytes], ch_or_dm_id: int, message: str, to_channel: bool) -> int:
    '''
...
def message_remove_v1(token: Union[str, bytes], message_id: int) -> dict:
    '''
...
def message_edit_v1(token: Union[str, bytes], message_id: int, edited_message: str) -> dict:
    '''
...
def message_share_v1(token: Union[str, bytes], og_message_id: int, optional_message: str, channel_id: int, dm_id: int) -> int:
    '''
    Given a message, update its text with new text. If the new message is an 
    empty string, the message is deleted.
...
def message_send_later_v1(token: Union[str, bytes], ch_or_dm_id: int, message: str, to_channel: bool, time_sent: int) -> int:
    '''
    Sends a message from an authorised user to the channel/dm specified.
    ...
    wait_time = time_sent - datetime.now().timestamp() 
...
def message_react_v1(token: Union[str, bytes], message_id: int, react_id: int) -> dict:
    '''
    ...
    if m_in_channel is not None or m_in_dm is not None:
        ...
        #The authorised user is not a member of the channel or DM that the message is within
        u_id = helper.check_token(token)['auth_user_id']
        if m_in_channel is None:
            m = m_in_dm[1]
            dm_id = m_in_dm[2]
            helper.user_in_dm_check(u_id, dm_id)
        else:
            m = m_in_channel[1]
            ch_id = m_in_channel[2]
            helper.user_in_channel_check(u_id, ch_id)
        ...
        #React a message
        m['reacts'][0]['u_ids'].append(u_id)
        m_uid = m['u_id']
        handle = helper.user_info(u_id)['handle_str']
        if m_in_channel is None:
            notify_react(u_id, m_uid, -1, dm_id, handle)
        else:
            notify_react(u_id, m_uid, ch_id, -1, handle)
...
def message_unreact_v1(token: Union[str, bytes], message_id: int, react_id: int) -> dict:
    '''
    ...
    if m_in_channel is not None or m_in_dm is not None:
        ...
        #Message with ID message_id does not contain an active React with ID react_id from the authorised user
        if u_id not in m['reacts'][0]['u_ids']:
            raise InputError(description="User did not reacted")

        #Unreact a message
        m['reacts'][0]['u_ids'].remove(u_id)
...
def message_pin_unpin_v1(token: Union[str, bytes], message_id: int, pin: bool) -> dict:
    '''
...
def message_errors(token: Union[str, bytes], message_id: int) -> tuple:
    '''
...
def check_message_tags(message: str, auth_user_id: int, channel_id: int, dm_id: int):
    '''
...
def add_message_to_db(message_info: dict, ch_or_dm_id: int, auth_user_id: int, to_channel: bool):
