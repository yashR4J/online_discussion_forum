...
def end_standup(channel_id: int):
...
def standup_start_v1(token: Union[str, bytes], channel_id: int, length: int) -> dict:
    '''
    ...
    t.start()
    return {'time_finished': datetime.now().timestamp()}


def standup_active_v1(token: Union[str, bytes], channel_id: int) -> dict:
    '''
    ...
    return {'is_active': channels[channel_id]['is_active'], 'time_finished': datetime.now().timestamp()}


def standup_send_v1(token: Union[str, bytes], channel_id: int, message: str) -> dict:
    '''
