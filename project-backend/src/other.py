...
def notify(auth_user_id: int, invitee_u_id: int, channel_id: int, dm_id: int, message: str, tagged: bool) -> None:
    '''
...
def get_notifications_v1(token: str) -> list:
    '''
...
def notify_react(auth_user_id: int, invitee_u_id: int, channel_id: int, dm_id: int, handle) -> None:
    '''
    Add a react notification to a user's list of notifications.
    ...
    message = f'{handle} reacted to your message in {ch_or_dm_name}'
    notification_info = {
        'channel_id' : channel_id,
        'dm_id' : dm_id,
        'notification_message' : message
